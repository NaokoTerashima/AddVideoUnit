...
namespace WinFormsAddVideoUnit
{
    public partial class Form1 : Form
    {
                ...
                while (csvRow < csvRowCount)
                {
                    ...
                    try
                    {
                        ...
                        while (cameraInt < cameraList.Count())
                        {
                            ...
                            var h264Streams = camera.Streams.Select(engine.GetEntity)
                                .OfType<VideoStream>()
                                .Where(s => s.VideoCompressionAlgorithm == VideoCompressionAlgorithmType.H264)
                                .OrderBy(s => ParseStreamSuffix(s.Name))
                                .ToList();
                            ...
                            if (h264Streams.Any())
                            {
                                var selectedStream = h264Streams.First();
                                var EntityStream = selectedStream; //It is replaced here.
        ...
        private static int ParseStreamSuffix(string name)
        {
            ...
            return int.MaxValue; // fallback if no suffix
        }
